...
def rectangle_subset_score_from_fit(
    pts4: np.ndarray,
    expected_rect_size: list[float] | tuple[float, float] | None = None,
    debug: bool = False,
) -> float:
    """
    Lower is better.
    Score each 4-hole subset by actually fitting the constrained rectangle model.
    Optional expected_rect_size = [size_x, size_y] in the same 2D plane units as the points.
    The order of size_x/size_y does not matter.
    """
    if pts4.shape != (4, 2):
        return float("inf")
    ...
    # Good square-like rectangles have four similar edges; this strongly rejects 6,6,12,17 cases.
    side_balance_err = np.std(L) / max(np.mean(L), 1e-9)

    size_prior_err = 0.0
    if expected_rect_size is not None:
        if len(expected_rect_size) != 2:
            raise RuntimeError("expected_rect_size must have exactly 2 values: size_x size_y")
        exp_a = 0.5 * float(expected_rect_size[0])
        exp_b = 0.5 * float(expected_rect_size[1])
        fitted_half = np.sort(np.abs(half_lengths_2d))
        expected_half = np.sort(np.array([exp_a, exp_b], dtype=np.float64))
        size_prior_err = float(np.sum(np.abs(fitted_half - expected_half) / np.maximum(expected_half, 1e-9)))
    ...
    if side_balance_err > 0.20:
        return float("inf")
    if expected_rect_size is not None and size_prior_err > 0.35:
        return float("inf")
    ...
    if debug:
        print(
            f"[DEBUG] fit={fit_err:.6e}, opp={opp_err:.6f}, diag={diag_err:.6f}, "
            f"ortho={ortho_err:.6f}, side_balance={side_balance_err:.6f}, "
            f"size_prior={size_prior_err:.6f}, total={score:.6f}"
        )

    return float(score)


def select_best_rectangular_4hole_subset(
    ...
    for idxs in itertools.combinations(range(len(centers_2d)), 4):
        ...
        score = rectangle_subset_score_from_fit(
            subset,
            expected_rect_size=expected_rect_size,
            debug=False,
        )
...
def compute_plane_axes_from_holes(
    ...
    support_thresh_ratio: float = 0.05,
    circularity_min: float = 0.30,
    expected_rect_size: list[float] | None = None,
    ...
    hole_centers_2d_used = select_best_rectangular_4hole_subset(
        hole_centers_2d_all,
        expected_rect_size=expected_rect_size,
        debug=debug,
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar=("SIZEX", "SIZEY"),
        help="Expected rectangle size in plane coordinates: size_x size_y",
    )
    ...
    result, dbg = compute_plane_axes_from_holes(
        ...
        expected_rect_size=args.expected_rect_size,
        crop=args.crop,
        debug=args.debug,
    )
